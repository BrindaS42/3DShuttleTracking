...
Optimized: Fast Chunk-Seek Trimming, Proper H_court Saving, and Correct 3D Coord Rendering.
"""
...
# ── Safe Imports from your project files ──
...
# ── Add BST stroke_classification to path ──
...
def compute_metrics(results_df: pd.DataFrame, out_dir: Path, logger: logging.Logger):
    ...
    eval_df = results_df[
        (results_df["gt_type"] != "接不到") & 
        (results_df["bst_pred_stroke"] != "SKIP")
    ].copy()
...
# ─────────────────────────────────────────────────────────────────────────────
#  SMART VIDEO TRIMMER (Fast Chunk-Seek)
# ─────────────────────────────────────────────────────────────────────────────
def create_rally_subset_video(video_path: str, gt_df: pd.DataFrame, n_total: int, out_path: str, logger: logging.Logger, buffer: int = 60, skip_extraction: bool = False):
    ...
    for i, (start_f, end_f) in enumerate(chunks):
        logger.info(f"  -> Extracting segment {i+1}/{len(chunks)} (Frames {start_f} to {end_f})...")
        ...
        while curr_frame <= end_f:
            ...
            if curr_frame % 100 == 0:
                print(f"     ... processing frame {curr_frame}/{end_f} ...", end='\r')
...
# ─────────────────────────────────────────────────────────────────────────────
#  HELPER: RUN MANUAL CALIBRATION VIA SUBPROCESS
# ─────────────────────────────────────────────────────────────────────────────
...
def draw_pose_frame(frame, pf_obj, coco_pairs, draw_coords=False):
    if not pf_obj:
        return
    ...
    for label, person in players:
        ...
        if hasattr(person, 'keypoints') and person.keypoints is not None:
            ...
            kpts = np.array(person['keypoints'])
        elif isinstance(person, (list, np.ndarray)):
            kpts = np.array(person)
        ...
        if kpts is not None and kpts.ndim >= 2 and kpts.shape[0] >= 17:
            ...
            if draw_coords:
                ...
                if hasattr(person, 'floor_pos_3d') and person.floor_pos_3d is not None:
                    court_x, court_y = person.floor_pos_3d[0], person.floor_pos_3d[1]
                elif isinstance(person, dict) and 'floor_pos_3d' in person and person['floor_pos_3d'] is not None:
                    court_x, court_y = person['floor_pos_3d'][0], person['floor_pos_3d'][1]
...
def dump_pose_video(video_path: str, trimmed_poses: list, out_path: str):
    ...
    while True:
        ...
        if frame_idx < len(trimmed_poses):
            draw_pose_frame(frame, trimmed_poses[frame_idx], coco_pairs, draw_coords=True)
...
def dump_final_annotated_video(trimmed_video_path: str, out_path: Path, trimmed_shuttle: np.ndarray, trimmed_poses: list, results_df: pd.DataFrame, frame_map: list, logger: logging.Logger):
    logger.info("Generating Final BST Annotated Video (Trimmed Version with Ground Truth)...")
    ...
    pred_dict = {
        row["frame_num"]: {
            "p_side": row["bst_pred_side"], 
            "p_stroke": row["bst_pred_stroke"],
            "gt_side": row["gt_side"],
            "gt_stroke": row["gt_mapped_stroke"]
        } 
    ...
    while True:
        ...
        if frame_idx < len(trimmed_poses):
            draw_pose_frame(frame, trimmed_poses[frame_idx], coco_pairs, draw_coords=False)
...
def main():
    ...
    # Target the 2nd shot of the first rally for calibration
    ...
    if len(first_rally_df) > 1:
        calib_target_frame = int(first_rally_df.iloc[1]["frame_num"])
    else:
        calib_target_frame = first_hit_frame
    ...
    # Check if the video is already trimmed and ask the user
    skip_trimming = False
    ...
    print("2. Parse from shuttleset/set/homography.csv")
    print("3. Recalculate using manual court calibration tool")
    ...
    if homo_mode == "3":
        ...
        logger.info(f"Saved manual homography matrix to {calib_dir / 'H_court.npy'}")
        
    elif homo_mode == "2":
        try: _, K, rvec, tvec = load_calibration(str(calib_dir))
        except Exception: 
            logger.warning("Need camera vectors for RTMPose. Opening manual calibration window...")
            run_manual_calibration(str(video_path), str(calib_dir), calib_target_frame, logger)
            _, K, rvec, tvec = load_calibration(str(calib_dir))
        ...
        logger.info("Loading cached calibration...")
        _, K, rvec, tvec = load_calibration(str(calib_dir))
        if (calib_dir / "H_court.npy").exists():
            H_court = np.load(calib_dir / "H_court.npy")
        else:
            H_court = build_court_homography(K, rvec, tvec)
    ...
    if use_pose_cache:
        ...
        while True:
            ...
            # Send exactly one batch to the pose estimator, then clear RAM
        ...
        # Process any leftover frames at the end of the video
    ...
    if input("Do you want to dump the annotated video for POSE ESTIMATION? (y/n): ").strip().lower() == 'y':
        dump_pose_video(trimmed_video_path, trimmed_poses, str(pose_dir / f"{match_folder}_pose_trimmed.mp4"))

    # Expand trimmed poses to global array
    ...
    if not weight_path.exists():
        logger.error(f"FATAL: Could not find model weights at: {weight_path.resolve()}")
        sys.exit(1)
    ...
    logger.info("Normalizing TRIMMED data (poses, shuttle, homography)...")
    ...
    for i, row in gt_df.iterrows():
        ...
        curr_rally = row["rally"]
        curr_set = row["set_file"]
        rally_hits = gt_df[(gt_df["rally"] == curr_rally) & (gt_df["set_file"] == curr_set)]["frame_num"].tolist()
        hit_pos = rally_hits.index(hit_idx)

        if hit_pos > 0 and rally_hits[hit_pos - 1] in frame_map:
            prev_h = frame_map.index(rally_hits[hit_pos - 1])
        else:
            prev_h = max(0, trimmed_hit_idx - 50)

        if hit_pos < len(rally_hits) - 1 and rally_hits[hit_pos + 1] in frame_map:
            next_h = frame_map.index(rally_hits[hit_pos + 1])
        else:
            next_h = min(n_trimmed - 1, trimmed_hit_idx + 50)
        ...
        # ── POST-PROCESSING: Side Matching & Fallback Filter ──
        for idx in sorted_idx:
            cand_full = all_bst_types[idx]
            cand_conf = float(probs[idx])
            
            if "_" in cand_full:
                cand_side, cand_stroke = cand_full.split("_", 1)
            else:
                cand_side = "Unknown"
                cand_stroke = "Unknown" if cand_full in ["Unknown", "未知球種"] else cand_full
                
            # Ignore 'Unknown' strokes if there are other valid choices
            if cand_stroke == "Unknown" and len(sorted_idx) > 1:
                continue
                
            # If we know the ground truth side, force the model to match it
            if expected_side in ["Top", "Bottom"]:
                ...
                # If GT side is unknown, just take the top non-unknown stroke
                pred_full, pred_side, pred_stroke, confidence = cand_full, cand_side, cand_stroke, cand_conf
                break
    ...
    if input("\nDump Final BST Annotated Video? (y/n): ").strip().lower() == 'y':
        dump_final_annotated_video(trimmed_video_path, shot_dir / f"{match_folder}_final_annotated_trimmed.mp4", trimmed_shuttle, safe_trimmed_poses, out_df, frame_map, logger)
