...
Replicates the highly-optimized check.py pipeline but replaces Stroke Classification 
with the Physics-Based Trajectory Solver.
"""
...
class EmptyPoseFrame:
    """Safe mock object to inject into pose lists if a player wasn't detected."""
    near = None
    far = None

# ─────────────────────────────────────────────────────────────────────────────
#  SMART VIDEO TRIMMER (Fast-Grab Mode)
# ─────────────────────────────────────────────────────────────────────────────
def create_rally_subset_video(video_path: str, gt_df: pd.DataFrame, n_total: int, out_path: str, logger: logging.Logger, buffer: int = 60, skip_extraction: bool = False):
    ...
    # ── FAST SEQUENTIAL READ (Keeps perfect sync, skips heavy decoding) ──
    while curr_frame <= max_frame:
        if curr_frame % 5000 == 0:
            logger.info(f"  -> Scanning frame {curr_frame} / {max_frame}...")
...
def dump_trajectory_video(trimmed_video_path: str, out_path: Path, master_traj_2d: np.ndarray, frame_map: list, fps: float, w: int, h: int, logger: logging.Logger):
    ...
    while True:
        ...
        # Draw a fading trail of the re-projected 3D coordinates
        if orig_f != -1:
...
def main():
    ...
    # Iterate through rallies safely
    for rally_id, group in gt_df.groupby(["set_file", "rally"]):
        ...
        for i in range(len(hits)):
            ...
            # The shot ends when the next player hits it, plus a 30 frame buffer for the final hit
            end_frame = hits[i+1]["frame_num"] if i < len(hits)-1 else min(start_frame + 30, n_total)
            
            gt_side = hits[i]["hitter_side_gt"]
            if gt_side not in ["Top", "Bottom"]:
                continue
                
            # Map Top/Bottom to far/near for pose extraction
            ...
            hitter_3d = get_player_3d(global_poses, start_frame, h_side, use_floor=False)
            
            recv_idx = min(end_frame - 1, n_total - 1)
            receiver_3d = get_player_3d(global_poses, recv_idx, other_side, use_floor=False)
