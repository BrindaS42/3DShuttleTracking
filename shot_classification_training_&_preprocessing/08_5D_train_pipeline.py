...
# Placeholder for your upcoming 5D model
# Ensure you create bst_5d.py and update the shuttle TCN input_dim to 5
try:
    from model.bst_5d import BST_CG_AP
except ImportError:
    print("⚠️ Warning: model.bst_5d not found. Ensure you create the file as planned.")
...
class Badminton5DDataset(Dataset):
    def __init__(self, data_path):
        ...
        # --- 5D FUSION LOGIC ---
        # Stack 2D (u,v) and 3D (x,y,z) to create (100, 5) vector
        s2d = np.array(data['s2d']) # (N, 100, 2)
        s3d = np.array(data['s3d']) # (N, 100, 3)
        s5d = np.concatenate([s2d, s3d], axis=-1) # (N, 100, 5)
        
        self.shut = torch.tensor(s5d, dtype=torch.float32)
...
def evaluate_comprehensive(model, loader, device, n_classes=35):
    """Calculates Acc, Acc@2, MacroF1, and MinF1 based on BST validation standards."""
    model.eval()
...
def main():
    ...
    best_5d_weight_file = ckpt_dir / "best_bst_5d_global.pt"
    resume_path = ckpt_dir / "latest_5d_checkpoint.pt"

    print(f"\n🚀 Starting 5D Training Pipeline (2D+3D Fusion) on: {device}")

    # 1. Load Pre-Split Cached Data
    if not (cache_dir / "train_data.pkl").exists():
        print("❌ Error: Global cache not found. Run 08_prepare_global_dataset.py first.")
        return

    train_loader = DataLoader(Badminton5DDataset(cache_dir/"train_data.pkl"), batch_size=64, shuffle=True)
    val_loader   = DataLoader(Badminton5DDataset(cache_dir/"val_data.pkl"), batch_size=64)
    test_loader  = DataLoader(Badminton5DDataset(cache_dir/"test_data.pkl"), batch_size=64)

    # 2. Initialize 5D Model
    # in_dim=72 for pose, internal shuttle processing handles 5-dim vector
    ...
    if resume_path.exists():
        print(f"🔄 Resuming 5D training from checkpoint: {resume_path}")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        model.train()
        epoch_loss = 0
        pbar = tqdm(train_loader, desc=f"5D Ep {epoch:03d}/{total_epochs}", leave=False)
        ...
        if metrics['acc'] > best_val_acc:
            ...
            early_stop_counter = 0
            torch.save({'model_state_dict': best_weights, 'metrics': metrics}, best_5d_weight_file)
            print(f"⭐ [New Best 5D] Acc: {metrics['acc']:.4f} | Acc@2: {metrics['acc2']:.4f} | mF1: {metrics['macro_f1']:.4f}")
        else:
            early_stop_counter += 1
            
        print(f"📊 [Val] Ep {epoch:03d} | Loss: {epoch_loss/len(train_loader):.4f} | "
              f"Acc: {metrics['acc']:.4f} | Acc@2: {metrics['acc2']:.4f} | mF1: {metrics['macro_f1']:.4f} | Patience: {early_stop_counter}/{early_stop_patience}")
        ...
        if early_stop_counter >= early_stop_patience:
            print(f"\n🛑 5D Early Stopping triggered at epoch {epoch}.")
            break

    # 5. Final Test
    print(f"\n{'='*70}\n🏁 5D TRAINING COMPLETE. LOADING BEST WEIGHTS FOR FINAL TEST...")
    if best_weights:
        model.load_state_dict(best_weights)
    
    test_metrics = evaluate_comprehensive(model, test_loader, device)
    print(f"🏆 FINAL 5D (FUSED) TEST RESULTS")
